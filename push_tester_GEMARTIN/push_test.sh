...
ARG=$(seq -250 249 | shuf | tr '\n' ' '); "$FILE" $ARG | ./pro_checker $ARG
exit 0
fi

if [ "$1" == -v2 ]; then
ARG=$2; "$FILE" $ARG | ./pro_checker $ARG
exit 0
fi
...
ARG="a"
"$FILE" $ARG > /dev/null 2> test_check.txt
if [ -s "$FICHERO" ];then
...
R=$("$FILE" $ARG | valgrind --leak-check=full ./checker_linux $ARG > /dev/null 2>&1)
R=$?
if [[ $R == 0 ]]; then
...
R=$(valgrind --leak-check=full "$FILE" $ARG > /dev/null 2>&1)
R=$?
((cont4++))
if [[ $R == 0 ]]; then
...
R=$("$FILE" $ARG | valgrind --leak-check=full ./checker $ARG > /dev/null 2>&1)
R=$?
if [[ $R == 0 ]]; then
  printf "${GREEN}[MOK] ${DEF_COLOR}";
else
