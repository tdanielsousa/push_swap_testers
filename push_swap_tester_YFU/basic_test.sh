...
# Run the push_swap program with Valgrind
(valgrind --leak-check=full --track-origins=yes $ROOT/push_swap $(cat $TRACE/error_files/test_case_$i.txt) 1>a 2>b) & pid=$!
(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
wait $pid 2>/dev/null;

# Check Valgrind output for memory leaks
grep "definitely lost" b > x
grep "indirectly lost" b >> x
grep "still reachable" b >> x
...
# Run the push_swap program with one argument
printf "\none argument\n\n"
($ROOT/push_swap "$(cat $TRACE/error_files/test_case_$i.txt)" >stdout 2>stderr) & pid=$!
(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
wait $pid 2>/dev/null;
...
rm -rf stdout stderr
	# Run the push_swap program with Valgrind
(valgrind --leak-check=full --track-origins=yes $ROOT/push_swap "$(cat $TRACE/identity_files/test_case_$i.txt)" 1>a 2>b) & pid=$!
(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
wait $pid 2>/dev/null;
...
# Print results for the identity test
...
for i in {1..5}; do
	...
	# Run the push_swap program with Valgrind
(valgrind --leak-check=full --track-origins=yes $ROOT/push_swap "$(cat $TRACE/size3/test_case_$i.txt)" 1>a 2>b) & pid=$!
(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
wait $pid 2>/dev/null;
...
# Print results for the size 3 test
printf "SIZE_3_TEST\n\nresult : $FLAG\nmemory : $LEAKFLAG\n\n\n"
...
(valgrind --leak-check=full --track-origins=yes $ROOT/push_swap "$(cat $TRACE/size5/test_case_$i.txt)" 1>a 2>b) & pid=$!
(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null & watcher=$!
wait $pid 2>/dev/null;
# Check Valgrind output for memory leaks
grep "definitely lost" b > x
